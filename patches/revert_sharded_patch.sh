#!/usr/bin/env bash
# revert_sharded_patch.sh - Restore original SGLang files on ACES
# Usage: ./revert_sharded_patch.sh [SGLANG_ROOT]
...
echo "=== Reverting Sharded Offload Patch ==="
echo "SGLang root: ${SGLANG_ROOT}"
echo "Backup dir:  ${BACKUP_DIR}"

if [ ! -d "${BACKUP_DIR}" ]; then
	echo "ERROR: No backups found at ${BACKUP_DIR}"
	echo "Was the patch applied with apply_sharded_patch.sh?"
	exit 1
fi
