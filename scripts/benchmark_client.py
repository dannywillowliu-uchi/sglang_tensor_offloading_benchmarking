...
Fixed to properly poll for async video completion.
"""
...
def poll_video_completion(base_url: str, video_id: str, poll_interval: float = 2.0, timeout: float = 600.0):
	"""Poll the video endpoint until completion or timeout."""
	...
	while True:
		...
		try:
			...
			if resp.status_code != 200:
				raise RuntimeError(f"Failed to get video status: {resp.status_code} - {resp.text}")
			...
			print(f"  Warning: polling error: {e}", flush=True)
...
def generate_video(
	...
	"""Send video generation request and wait for completion."""
	...
	if resp.status_code != 200:
		if gpu_sampler:
			gpu_sampler.stop()
		raise RuntimeError(f"Failed to submit video job: {resp.status_code} - {resp.text}")
	...
	if not video_id:
		if gpu_sampler:
			gpu_sampler.stop()
		raise RuntimeError(f"No video ID in response: {job_data}")

	print(f"  Job submitted: {video_id}")

	# Poll for completion
	try:
		result = poll_video_completion(base_url, video_id, poll_interval, timeout)
	finally:
		# Stop GPU sampling
		if gpu_sampler:
			gpu_sampler.stop()
...
def run_benchmark(
	...
	Default parameters match PR #15511 benchmark:
	...
	print(f"Guidance scales: {guidance_scale}, {guidance_scale_2}")
	...
	for i in range(num_warmup):
		...
		result = generate_video(
			...
			gpu_sampler=gpu_sampler, poll_interval=poll_interval, timeout=timeout
		)
...
if __name__ == "__main__":
	parser = argparse.ArgumentParser(
		description="Benchmark SGLang video generation (defaults match PR #15511)"
	)
	...
	parser.add_argument("--prompt", default="A cat walks on the grass, realistic",
		help="Prompt for video generation (PR #15511 default)")
	parser.add_argument("--width", type=int, default=1280, help="Video width (PR #15511: 1280)")
	parser.add_argument("--height", type=int, default=720, help="Video height (PR #15511: 720)")
	parser.add_argument("--num-frames", type=int, default=81, help="Number of frames (PR #15511: 81)")
	parser.add_argument("--num-inference-steps", type=int, default=27, help="Inference steps (PR #15511: 27)")
	parser.add_argument("--guidance-scale", type=float, default=3.5, help="Guidance scale (PR #15511: 3.5)")
	parser.add_argument("--guidance-scale-2", type=float, default=4.0, help="Guidance scale 2 (PR #15511: 4.0)")
	...
	parser.add_argument("--timeout", type=float, default=600.0, help="Timeout per generation in seconds")
